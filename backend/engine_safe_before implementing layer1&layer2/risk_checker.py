...
def run_risk_checks(
    ...
    # ── ITR form routing flags ──
    if profile.itr_form == "ITR-3":
        flag(
            "ITR3_EXPERT_PATH", "warning",
            "ITR-3 requires regular books of account. Expert/CA review is recommended "
            "before filing."
        )
    if profile.itr_form == "ITR-4" and user.business.business_type != "none":
        flag(
            "ITR4_PRESUMPTIVE", "info",
            f"ITR-4 presumptive income under Section {user.business.business_type}. "
            "Maintain turnover/receipt records and bank statements as proof."
        )
    if profile.expert_required:
        flag(
            "EXPERT_ESCALATION", "warning",
            "Complex income profile — CA escalation recommended before filing."
        )
