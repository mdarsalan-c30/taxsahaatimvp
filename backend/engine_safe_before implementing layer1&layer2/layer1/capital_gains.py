...
def compute_capital_gains(cg: CapitalGainsInput) -> dict:
    ...
    ltcl_remaining = ltcl
    ltcg_112a_net = max(0.0, ltcg_112a_net - ltcl_remaining)
    ltcl_remaining = max(0.0, ltcl_remaining - ltcg_112a_net)

    ltcg_other_net = max(0.0, ltcg_other_net - ltcl_remaining)
    ltcl_remaining = max(0.0, ltcl_remaining - ltcg_other_net)
