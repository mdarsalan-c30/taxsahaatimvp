...
=============================================
Tests and demonstrates the full Layer 1 + Layer 2 pipeline (v3).

File map (v3, 8 files total)
-----------------------------
    models.py        dataclasses shared by both layers
    engine.py         Layer 1 — deterministic tax math (FY2025-26/AY2026-27)
    layer2.py         Layer 2 — LLM deduction discovery (Anthropic API)
    serializers.py    JSON <-> dataclass conversion
    service.py        "Python Tax Engine" CLI — stdin/stdout JSON, called
                       via subprocess from the Next.js API backend
    api.py            same logic as a persistent FastAPI HTTP service
    run.py            this file
    tests.py          81-test suite (tax-math, integration, invariants)
...
def print_l1(r):
    ...
    print(f"  GTI (old/new)     : {inr(r.income_heads.gross_total_income)} / {inr(r.income_heads.gross_total_income_new_regime)}")
